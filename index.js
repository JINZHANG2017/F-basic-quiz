// TODO GTB-工程实践: * 很多方法不符合单一职责，建议按照业务逻辑划分模块或方法
// TODO GTB-工程实践: * 不要去disable eslint，这通常说明代码工程实践存在问题
// TODO GTB-完成度: * 大部分功能已实现，样式需要微调
/* eslint-disable no-console */
/* eslint-disable no-alert */
/* eslint-disable func-names */
/* eslint-disable no-undef */
function fetchData(url) {
  // TODO GTB-工程实践: - 代码不需要注释，通过代码本身来体现业务功能
  // <-- start
...
// TODO GTB-知识点: - 不推荐jquery调用函数，声明函数后，直接函数调用就可以了
$(function () {
  // TODO GTB-工程实践: - 长方法，建议拆分逻辑
  const currentUrl = window.location.href;
  const splitArray = currentUrl.split("/");
  //   console.log(splitArray);
  // TODO GTB-完成度: - 需要检查URL访问，当请求/any/1时，不应返回user
  const requestId = splitArray[splitArray.length - 1];
  const url = `http://localhost:8080/users/${requestId}`;
  fetchData(url)
    .then((result) => {
      if (result.message) {
        // TODO GTB-知识点: - 应该通过请求status code去判断请求是否成功，而不是通过message
        console.log(result.message);
        return;
      }
      console.log(result);
      // TODO GTB-知识点: -  user和educations数据是两个请求，可以分别获取，获取后独立渲染
      ...
      result.educations.forEach((value) => {
        ...
        // TODO GTB-知识点: -  在for循环中，每次都要操作DOM获取元素，影响性能，不推荐这样做
